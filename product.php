...
<li class="mx-left" style="width:20% !important">
<?php
// Show all information, defaults to INFO_ALL
	foreach (file("navigator.txt") as$line){
	$arr = explode(' ',trim($line));
...
?>
  ...
  <!-- <li>Skincare Product</li>
  <li>MakeUp Product</li>
  <li>Skincare tools</li> -->
  <li class="mx-left" style="width:20% !important">
  <?php 
  foreach(file("product.txt") as$line){
  	$productarr = explode(' ', trim($line));
  	echo "<li><a class = xh href = '$productarr[0]'> {$productarr[1]} </a></li>";
  	//echo('&nbsp;');
  }
  ?>
  </li>
